...
export class RealBlendProtocolAdapter implements BlendAdapter {
  ...
  private initializePoolContracts() {
    ...
    this.poolContracts.set(
      'yieldblox_admin',
      'CANSYFVMIP7JVYEZQ463Y2I2VLEVNLDJJ4QNZTDBGLOOGKURPTW4A6FQ'
    );
  ...
  private async fetchPoolDataWithFallback(
  ...
  private async fetchUserPosition(
    server: StellarSdk.SorobanRpc.Server,
    contractAddress: string,
    userAddress: string
  ): Promise<Omit<UserPosition, 'poolId'>> {
